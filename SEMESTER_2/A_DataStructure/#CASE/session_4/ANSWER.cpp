...
void insert(){
    ...
    while(error){
        ...
        if(strlen(title) >= 5 && strlen(title) <= 50 && isunique(title)) error = false;
    }
    ...
    while(error){
        ...
        if((strlen(author) >= 3 && strlen(author) <= 25) && (strncmp(author, "Mr.", 3) == 0|| strncmp(author, "Mrs.", 4) == 0)) error = false;
    }
    ...
    while(error){
        ...
        if(strlen(ISBN) >= 10 && strlen(ISBN) <= 13 && isalldigit(ISBN)) error = false;
    }
    ...
    while(error){
        printf("Page : ");
        scanf("%d", &pnum); getchar();

        if(pnum >= 16) error = false;
    }
